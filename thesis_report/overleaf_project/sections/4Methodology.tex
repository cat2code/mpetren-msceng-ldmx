...
%TODO: the math below is too advanced and must be simplified, maybe just stated in words instead. 
\begin{align}
    D_{ij}
    &=
    \sum_{k=1}^{K_i}
...
contribution from the previous \gls{ecal} cell. The summed rule was adopted because all contributions from the same physical origin should, from a physics motivated standpoint, be combined before the dominant origin is chosen, interestingly though no impact on model performance found. 
...
and mark them with a Boolean noise target. Here, ``noise'' therefore denotes
this simulation-defined category rather than an estimate of unmodelled noise
in detector data. A noise hit is assigned to background class
...
Each temporary event record is converted into numerical arrays, masks, targets and metadata. Events keep their variable size and the stored order within each detector collection. 
