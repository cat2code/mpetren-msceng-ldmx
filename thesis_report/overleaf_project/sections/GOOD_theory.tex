...
Pile-up is not only a reconstruction complication. If multi-electron events
can be retained, it is also an opportunity to increase the useful electron
throughput. The LDMX design report
...
\subsection{Machine Learning for Set-Based Detector Reconstruction}
\label{sec:ml_set_reconstruction}

%\subsection{Why ML?} I think I need more subsections here
% I need some more motivation than what I have right now... 

\subsubsection{Learning a Reconstruction Mapping}

The range of \gls{ml} applications in \gls{hep} has expanded
rapidly \cite{feickert2021-livingreview,iml-wg-hepml-livingreview}. Popularity
alone is not a scientific motivation for using \gls{ml}, its relevance here
is rather that simulation supplies paired detector observations and
particle-level truth provides a natural application for classification tasks to be explored, while overlapping showers create associations that are
difficult to express through a set of geometric rules designed ''by hand''. 


The same idea in Equation~\ref{eq:pileup_detector_response} can be stated more generally for an intuitive illustration for the reader. Let \(Y\) be the generated
particles in an event, let \(\mathcal{S}\) be the detector simulation, and let
\(X\) be the detector signals produced by that simulation. Following the
functional description used for \gls{mlpf} \cite{pata2021-mlpf}, detector simulation
maps \(Y\) to \(X\), while reconstruction tries invert and to map \(X\) back to \(Y\):
\begin{equation}
    \mathcal{S}(Y)=X,
    \qquad
    \mathcal{R}_{\theta}(X)
    \simeq
    \mathcal{S}^{-1}(X)=Y,
    \label{eq:learned_reconstruction_mapping}
\end{equation}
Here, \(\mathcal{R}_{\theta}\) is the learned reconstruction model, where $\theta$ denotes the learnable parameters. The
inverse notation is conceptual rather than literal.
The forward map is stochastic and loses information through finite detector
granularity, thresholds and noise, so an exact and unique
\(\mathcal{S}^{-1}\) generally does not exist. 
%For a task-specific truth quantity \(T(Y)\), such as a hit-origin label or energy fraction, their actual objective is \(\mathcal{R}_{\theta}(X)\approx T(Y)\).

Rule-based reconstruction encodes detector knowledge through explicit
operations, while learned reconstruction estimates a parameterised mapping
from examples. The latter can combine many correlated inputs without every
decision rule being specified manually, but it is not automatically optimal
and depends on the simulation, target definition and training distribution, so
the approaches are complementary. The models in this thesis receive
ECal hits and trigger-pad tracks that have already passed through conventional
detector reconstruction, and learn an additional association and prediction
stage on top of those objects.


\subsubsection{Machine-Learned Particle-Flow as a Reference Case}

Particle-flow reconstruction combines complementary tracking and calorimeter
measurements to reconstruct individual final-state particles. \Gls{mlpf},
developed for the \gls{cms} experiment, replaces parts of this association and
particle-level prediction procedure with a learned model, making it a relevant
example of machine learning used inside reconstruction rather than only as a
final event classifier. The original study used a \gls{gnn}
architecture under high-pile-up conditions \cite{pata2021-mlpf}. The latest iteration from the start of this year transitions the architecture to a Transformer integrated into the \gls{cms} reconstruction
software and evaluates it on both simulation and collision data
\cite{pata2026-mlpf}. The transition also
illustrates the growing importance of accelerator\footnote{``Accelerator'' in this context refers to specialized hardware such as \glspl{gpu} or even \gls{ml} specialized architectures such as the relatively new \gls{tpu} that accelerate the speed of computation for applicable processes \cite{sur2026-heterogeneous-computing}}-friendly computation for high-dimensional, variable-size event data.

\gls{ml} and rule-based reconstruction are complementary rather than
exclusive. \gls{mlpf} receives calorimeter clusters, \textit{detector objects}, that already include outputs from
earlier reconstruction stages, then learns associations and particle-level
predictions from those objects. The high occupancy and heterogeneous inputs of \gls{cms} particle flow
are substantially broader than the problem studied here. The present thesis studies a narrower
LDMX-specific problem to explore: finding how overlapping electron showers can be separated at hit
level, but adopts the same general view of an event as a variable-size collection of heterogeneous detector objects.
