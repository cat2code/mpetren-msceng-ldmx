...
their name in the \gls{ldmx} codebase, \texttt{TriggerPadTracks}, or simply
\gls{tpad} in flowing text. The term describes objects reconstructed from TS
measurements, not a separate detector. Each candidate provides a
one-dimensional position estimate and a photoelectron count. Taken together,
the candidates provide information about the positions and multiplicity of
the incoming electrons. They are detector observations rather than
simulation-truth labels and only correspond one-to-one with the incident
electrons when the rule-based reconstruction succeeds
\cite{ldmx2025-overview}.
...
This latency requirement distinguishes online from offline reconstruction.
Online algorithms operate before storage with strict limits on time, memory
and data transfer, using quantities such as ECal trigger-cell sums and
reconstructed TS candidates. After an accepted event has been fully read out,
offline algorithms can use individual ECal cells and more computationally
demanding methods. The models developed in this thesis use full-resolution
simulated ECal hits and therefore address offline reconstruction. Adapting
them to the trigger would require a separate study with trigger-level inputs,
...
practical challenge is therefore to benefit from a higher beam intensity
...
negligible. This makes it possible to generate their particle histories
separately and overlay them before detector digitisation and reconstruction.
Let \(Z^{(k)}\) denote the simulated deposits from electron \(k\). The
observed hit collection can be written schematically as
\begin{equation}
    \mathcal{H}
    =
    \mathcal{R}\!\left(
...
general recover the original \(Z^{(k)}\).
...
of origin. The experimental multi-task model also infers the restricted
...
Multi-electron buckets are not only a reconstruction complication. They can
also increase the useful electron throughput. The LDMX design report
...
practical reach of the experiment \cite{ldmx2025-overview}.
...
simulated \(2e\) and \(3e\) events. It does not calculate a change in LDMX
sensitivity or optimise the online trigger. Instead, it asks whether modern
...
The pile-up problem combines a variable number of irregular detector objects
whose interpretation depends on their relations to the rest of the event.
This motivates models that process an event as an unordered set and learn
object-to-object interactions. The following subsections introduce a recent
particle-flow example and the GravNet and Transformer architectures used in
this thesis.
...
use a pre-layer-normalised encoder block,
\begin{align}
    \widetilde{\mathbf{H}}
    &=
    \mathbf{H}
...
attention update and the pre-layer-normalised block used in this thesis.
...
A Transformer encoder can be interpreted within the generic message-passing
framework of a \gls{gnn}. Tokens become nodes in a fully connected graph, and
self-attention determines the weight of every connection. Each node then
exchanges information with every other node, including itself, just as each
Transformer token attends to every valid token. Under this interpretation, a
Transformer is a message-passing network whose graph is complete and whose
edge weights are learned through attention \cite{joshi2025transformersgnn}.
...
Transformers benefit from a hardware and software ecosystem that is highly
optimised for the dense matrix multiplications used by attention. This is an
example of the \emph{hardware lottery}: an architecture can gain a practical
advantage when its operations align well with available accelerators and
software libraries, without this making it universally superior
\cite{hooker2020hardwarelottery,joshi2025transformersgnn}. In practice, all
...
software libraries \cite{fey2019-pytorch-geometric}. Full self-attention is
not mathematically cheaper because the number of token pairs grows as
\(N^2\), compared with \(Nk\) edges after the GravNet graph has been
constructed. It may nevertheless execute faster for a particular workload.
The practical comparison depends on event size, batch size, implementation
and hardware, and is therefore measured empirically in the Results and
Discussion chapter.
