...
different trigger-pad objects onto one regular image. A \gls{cnn} is naturally biased towards a fixed 2D grid and local translation
symmetry, whereas the \gls{ecal} detector has a hexagonal grid structure, the \gls{ldmx} has information from heterogoneous detectors and the number of detector objects vary between
...
information \cite{duarte2020-\gls{gnn}-tracking,lee2019-set-transformer}.
...
principle is unchanged \cite{gilmer2017-message-passing,duarte2020-\gls{gnn}-tracking}.
...
Only the Transformer \textit{encoder} is used, the autoregressive decoder from the
original language-model architecture \cite{vaswani2017attention} is unnecessary. Sequence-position
encoding is also omitted because detector-row order is arbitrary. Physical
position enters explicitly through detector-coordinate features. %Shorter events are padded within a mini-batch, and an attention mask prevents padded rows from sending or receiving information. 
